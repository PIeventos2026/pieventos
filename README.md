# Eventos Fatec

## 📌 Objetivo

O projeto **Eventos Fatec** foi desenvolvido com o objetivo de criar uma plataforma institucional para divulgação e organização de eventos acadêmicos da FATEC.  

A aplicação busca facilitar o acesso às informações dos eventos, permitindo uma navegação simples, moderna e intuitiva para alunos, professores e visitantes.
## Tecnologias
<p align="left"> <img src="https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white"> <img src="https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white"> <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black"> </p>

##Autores

Laura Emerich - Desenvolvimento Full Stack e estrutura do projeto - @Eemerichlaura
Vitória Letícia Fernandes - Desenvolvimento Front-End e Auxílio na Interface - @VitoriaLe141
Yasmin Rodrigues Astori -Desenvolvimento do Canva e Mapa Interativo - @yasminastori017-cpu
Sara Lorraine Martins -Testes e Apoio ao Projeto - @saralorrainemartins-pixel

## 📄Licença

Este projeto foi desenvolvido para fins acadêmicos na FATEC.
